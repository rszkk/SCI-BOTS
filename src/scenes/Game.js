...
export default function Game (playerInfo) {
    ...
    const completedLevelInnerIndex = playerInfo.completedLevelsInfo[playerInfo.choosedLevel][0].timeEnd ? 1 : 0;
    console.log('versao ->', completedLevelInnerIndex)
    let playerSpeed = 200;
    ...
    let canEat = false;
    let isDead = false;
    let movements = playerInfo.completedLevelsInfo[playerInfo.choosedLevel][completedLevelInnerIndex].movements ?? 0;
    let score = playerInfo.completedLevelsInfo[playerInfo.choosedLevel][completedLevelInnerIndex].score ?? 0;
    let attempts = playerInfo.completedLevelsInfo[playerInfo.choosedLevel][completedLevelInnerIndex].attempts;
    ...
    document.querySelectorAll('.al').forEach((item) => {
        let index = item.classList[item.classList.length-1].split('-')[1]
        if(maps[playerInfo.choosedLevel][1].actionListAvailble 
            && index >= maps[playerInfo.choosedLevel][1].actionListAvailble+1) item.style.display = 'none';
    ...
    const levelSuccessModal = () => {
        ...
        const back = k.add([
            k.rect(120, 50),
            k.pos(k.width() * 0.5 - 175, k.height() * 0.5 + 105),
            k.layer('modalBtn'),
            k.color(k.BLACK),
            k.area(),
            k.opacity(0.9),
        ...
        const deathsText = k.add([
            k.pos(k.width() * 0.5, 285),
            k.text(`Attempts: ${attempts}`, {
                size: 24,
                font: 'sink',
            }),
            k.origin('center'),
        ...
        const stars = k.add([
            ...
            k.pos(k.width() * 0.5, 365),
            k.origin('center'),
            k.layer('modalText'),
            k.scale(1.2),
        ]);
        if (score !== 3) {
            const confrontText = k.add([
                k.pos(k.width() * 0.28, 192),
                k.text('You can do better!! :D', {
                    size: 16,
                    font: 'sink',
                }),
                k.color(k.CYAN),
        ...
        if (maps[playerInfo.choosedLevel+1]){
            const next = k.add([
                k.rect(120, 50),
                k.pos(k.width() * 0.5 + 55, k.height() * 0.5 + 105),
                k.layer('modalBtn'),
                k.color(k.BLACK),
                k.area(),
                k.opacity(0.9),
