...
export default function Game (playerInfo) {
    ...
    const gameLevel = k.addLevel(maps[1], levelCfg);  //playerInfo.choosedLevel
    ...
    const enemy = k.add([
        k.sprite('fly-food'),
        k.origin('center'),
        k.pos(60 + 30, 180 + 30), // teste right 1
    ...
    const hasFloor = (x, y, angle) => {
        ...
        return maps[1][yValue-1]?.split('')[xValue-1] === 'a';
    }
        ...
        if (!hasWonLevel) {
            await new Promise((resolve, reject) => {
                setTimeout(() => {
                    k.gravity(1000)
                    k.destroy(enemy)
                    // rotate('faceUp') arrumar pra nao bugar os modais
                    failureModal('died')
                    resolve()
                }, 100)
            })
    ...
    playButton.addEventListener('click', () => {
        ...
        setTimeout(async () => {
            ...
            // if (!hasWonLevel) {
            //     setTimeout(() => console.log('lost'), 300);
            //     // setTimeout(() => failureModal('lost'), 300);
            // }
        }, 500);
    ...
    const levelSuccessModal = () => {
        ...
        const modalText = k.add([
            k.pos(k.width() * 0.51, 215),
            k.text(`Level concluido!`, {
                size: 30,
                font: 'sink',
            }),
            k.origin('center'),
