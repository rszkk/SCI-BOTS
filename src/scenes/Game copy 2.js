...
export default function Game (playerInfo) {
    ...
    function goRight () {
        return new Promise((resolve, reject) => {
            if (player.angle === 0) {
                // console.log('r1')
                let target = player.pos.x + 60;
                console.log(roundToNearest60(player.pos.x))
                const timer = setInterval(() => {
                    if (didHitWall) {
                        ...
                        if (player.pos.x > target) {
                            const end = floorTest(roundToNearest60(player.pos.x), player.pos.y+30, 0);
                            if (end) {
                                clearInterval(timer);
                ...
                const timer = setInterval(() => {
                    if (didHitWall) {
                        ...
                        if(player.pos.y < target) {
                            // console.log(player.pos.x+30, roundToNearest60(player.pos.y))
                            const end = floorTest(player.pos.x+30, roundToNearest60(player.pos.y), 270);
                            if (end) {
                                clearInterval(timer);
                ...
                const timer = setInterval(() => {
                    if (didHitWall) {
                        ...
                        if (player.pos.y > target) {
                            const end = floorTest(player.pos.x+30, roundToNearest60(player.pos.y))
                            if (end) {
                                clearInterval(timer);
                                player.moveTo(player.pos.x, roundToNearest60(player.pos.y)-30)
                                playerDie()
                ...
                const timer = setInterval(() => {
                    if (player.pos.x <= target) {
                        const end = floorTest(roundToNearest60(player.pos.x-1), player.pos.y+30, 180)
                        if (end) {
                            playerDie()
                            clearInterval(timer);
                            resolve();
