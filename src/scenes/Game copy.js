...
export default function Game (playerInfo) {
    ...
    function goRight (hasNextStep) {
        return new Promise((resolve, reject) => {
            if (player.angle === 0) {
                // console.log('r1')
                const target = player.pos.x + 60;
                const timer = setInterval(() => {
                    if (player.pos.x > target || player.angle !== 0 || didHitWall) {
                        clearInterval(timer);
                        // console.log(player.pos)
                        // if (!didHitWall) console.log('a1', roundToNearest60(player.pos.x+30) - 30, player.pos.y)
                        if (!didHitWall) player.moveTo(roundToNearest60(player.pos.x+30) - 30, player.pos.y);
                        else checkPlayerFacingPos('right');
                        resolve();
                    } else moveRight();
                ...
                const timer = setInterval(() => {
                    if (player.pos.y < target || player.angle !== 270 || didHitWall) {
                        clearInterval(timer);
                        // console.log(player.pos)
                        // if (!didHitWall) console.log('a2', player.pos.x, roundToNearest60(player.pos.y - 30)+30)
                        if (!didHitWall) player.moveTo(player.pos.x, roundToNearest60(player.pos.y -30)+30);
                        else checkPlayerFacingPos('right');
                        resolve();
                    } else moveUp();
                }, 7)
                ...
                const target = player.pos.y + 60;
                const timer = setInterval(() => {
                    if (player.pos.y > target || player.angle !== 90 || didHitWall) {
                        clearInterval(timer);
                        // console.log(player.pos)
                        // if (!didHitWall) console.log('a3', player.pos.x, roundToNearest60(player.pos.y + 30)-30)
                        if (!didHitWall) player.moveTo(player.pos.x, roundToNearest60(player.pos.y +30)-30);
                        else checkPlayerFacingPos('right');
                        resolve();
                    } else moveDown();
                ...
                const target = player.pos.x - 60;
                const timer = setInterval(() => {
                    if (player.pos.x < target || player.angle !== 180 || didHitWall) {
                        clearInterval(timer);
                        // console.log(player.pos)
                        // if (!didHitWall) console.log('a4', roundToNearest60(player.pos.x -30) + 30, player.pos.y)
                        if (!didHitWall) player.moveTo(roundToNearest60(player.pos.x -30) + 30, player.pos.y);
                        else checkPlayerFacingPos('right');
                        resolve();
                    } else moveLeft();
    ...
    function goLeft (hasNextStep) {
        return new Promise((resolve, reject) => {
            if (player.angle === 0) {
                console.log('l1', didHitWall)
                const target = player.pos.x - 60;
                const timer = setInterval(() => {
                    if (player.pos.x < target || player.angle !== 0 || didHitWall) {
                        clearInterval(timer);
                        if (!didHitWall) player.moveTo(roundToNearest60(player.pos.x-30) + 30, player.pos.y)
                        // if (didHitWall && hasNextStep) checkPlayerFacingPos('left');
                        resolve();
                    } else moveLeft()
                }, 7)
            } else if (player.angle === 270) {
                console.log('l2', didHitWall)
                const target = player.pos.y + 60;
                const timer = setInterval(() => {
                    if (player.pos.y > target || player.angle !== 270 || didHitWall) {
                        clearInterval(timer);
                ...
                console.log('l3', didHitWall)
                const target = player.pos.y - 60;
                const timer = setInterval(() => {
                    if (player.pos.y < target || player.angle !== 90 || didHitWall) {
                        clearInterval(timer);
                        console.log('didHitWall', didHitWall)
                        if (!didHitWall) player.moveTo(player.pos.x, roundToNearest60(player.pos.y - 30) + 30)
                        // if (didHitWall && hasNextStep) checkPlayerFacingPos('left');
                        resolve(didHitWall);
                    } else moveUp()
            ...
            if (didHitWall) didHitWall = false;
        });
    }
    ...
    async function executeMovements (actionList) {
        ...
        for await (const action of actionList) {
            ...
            console.log('aux',aux)
            aux += 1;
            let hasNextStep = actionList[aux] === 'left' || actionList[aux] === 'right';
            console.log('next step ->', hasNextStep)
            if (teste) {
                console.log('entrou');
            ...
            if (action === 'left') teste = await goLeft();
            if (action === 'jump') {
                didHitWall = false;
                await jump();
                didHitWall = false;
